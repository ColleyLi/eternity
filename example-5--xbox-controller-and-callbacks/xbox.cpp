...
///////////////////////////////////////
// xbox 360 EXAMPLE CODE, with callbacks //
///////////////////////////////////////
...
#pragma region callback sample vars and functions
// A gamestate variable for the eye's
// interaction with the player
bool callbackSet = false ; // has the "bomb been set?"

bool callbackRAN = false ; // this gets set to TRUE
// when the callback has started running, i.e.
...
void Update()
{
  ...
  {
    // if its plugged in and the callback
    // hasn't been set yet, then SET
    // THE CALLBACK
    if( ! callbackSet )
    {
      window->addCallback(
        new Callback0( 5.0, startVibrate )
      ) ;

      // This is sort of metaphorically
      // equivalent to planting a time-bomb.
      // The callback will "occur" in
      // 5.0 seconds from when the above line
      // of code runs, so in that sense
      // "the bomb is set"
      callbackSet = true ;
    }
  }
...
void Draw()
{
  ...
  if( window->gamepadIsPresent( Gamepad::One ) )
  {
    ...
    if( !callbackRAN )
    {
      window->drawSprite( Sprites::Eye, 320, 240, 200, 200 ) ;

      window->drawString( Fonts::Arial24,
        "I see your controller.\n"
        "Why don't you rest your hands\n"
    ...
    else
    {
      ...
      angle = -angle ;
      window->drawSprite( Sprites::Eye,
        320, 240, 240, 240, angle, Color::Red ) ;

      window->drawString( Fonts::Arial24,
        "BLAAAARAAAARAARAARRRRRRRG!!!!!",
        Color::Red,
        440, 240, 120, 200, DT_LEFT | DT_TOP | DT_WORDBREAK ) ;
    }
  ...
  else
  {
    ...
    window->drawString( Fonts::Arial24,
      "I see no controllers.  Connecteth one, NOW!!",
      Color::Red,
      440, 240, 120, 200, DT_LEFT | DT_TOP | DT_WORDBREAK ) ;
  }
