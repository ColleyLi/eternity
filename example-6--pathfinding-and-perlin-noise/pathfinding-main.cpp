...
Coord startCoord ; // the start pos is the beginning of the map
Coord endCoord ;   // the end position
...
void Init()
{
  ...
  startCoord.row = randInt( 0, asciiMap->getRows() ) ;
  startCoord.col = randInt( 0, asciiMap->getCols() ) ;
  
  // random end 
  endCoord.row = randInt( 0, asciiMap->getRows() ) ;
  endCoord.col = randInt( 0, asciiMap->getCols() ) ;
...
void Update()
{
  ...
  solutionPath = asciiMap->solve( startCoord, endCoord ) ;
  ...
  startCoord.clampBetween( 0, asciiMap->getRows()-1, 0, asciiMap->getCols()-1 ) ;
  endCoord.clampBetween( 0, asciiMap->getRows()-1, 0, asciiMap->getCols()-1 ) ;
...
void Draw()
{
  ...
  x = offsetX + (startCoord.col * tileSize) ;
  y = offsetY + (startCoord.row * tileSize) ;
  window->drawSprite( Sprites::StartNode, x, y ) ;
  
  x = offsetX + (endCoord.col * tileSize) ;
  y = offsetY + (endCoord.row * tileSize) ;
